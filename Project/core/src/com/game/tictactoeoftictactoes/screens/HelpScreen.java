...
public class HelpScreen extends GameScreen {
    ...
    public HelpScreen(final com.game.tictactoeoftictactoes.TicTacToeGame game) {
        ...
        menuButton.setPosition(Gdx.graphics.getWidth() / 2 - menuButton.getWidth() / 2, Gdx.graphics.getHeight() / 10);
    ...
    private void refreshRootTable(int width, int height){
        ...
        root.row();
        root.setPosition(Gdx.graphics.getWidth() / 2, Gdx.graphics.getHeight() / 1.4f);
    ...
    private void restartAnimation(){
        ...
        this.mapGrid = new com.game.tictactoeoftictactoes.objects.MapGrid(Gdx.graphics.getWidth(), Gdx.graphics.getHeight(), fieldSize, game, this);
        mapGrids = new com.game.tictactoeoftictactoes.objects.MapGrid[fieldSize][fieldSize];
        winnerFields = new Vector2[fieldSize];
        winnerTime = new float[fieldSize][fieldSize];
        for (int i = 0; i < fieldSize; i++) {
            for (int j = 0; j < fieldSize; j++) {
                mapGrids[i][j] = new com.game.tictactoeoftictactoes.objects.MapGrid(mapGrid.getX0() + j * mapGrid.getCELL_SIZE() + mapGrid.getCELL_SIZE() / 11
